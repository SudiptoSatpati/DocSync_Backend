...
import { addUserToDocument, removeUserFromDocument, getOnlineUsers, hasDocumentAccess } from '../utils/helper';
...
export const initSocketService = (io: Server) => {
    io.use(async (socket: Socket, next) => {
        try {
            ...
            console.error('Socket authentication middleware error:', (error as Error).message);
    ...
    io.on('connection', async (socket: Socket) => {
        ...
        // 
         ...
         socket.on('get-document', async (documentId: string) => {
            try {
                console.log(`🛠️ ${user.username} is attempting to access document: ${documentId}`);
                ...
                console.log(`✅ ${user.username} successfully loaded document: ${documentId}`);
                
                // Debug log users in the document
                setTimeout(() => {
                    const room = io.sockets.adapter.rooms.get(documentId);
                    if (room) {
                        console.log(`👥 Users in document ${documentId}:`, Array.from(room));
                    } else {
        ...
        socket.on('send-changes', async (delta: any) => {
            try {
                ...
                for (const documentId of rooms) {
                    ...
                    const isOwner = document.get('owner').toString() === user._id.toString();
                    ...
                    console.log(`📝 ${user.username} sent changes to document ${documentId}`);
                }
            } catch (error) {
        ...
        socket.on('cursor-position', ({ documentId, position }: { documentId: string; position: any }) => {
        ...
        socket.on('save-document', async (data: any) => {
            try {
                ...
                for (const documentId of rooms) {
                    ...
                    console.log(`💾 Document ${documentId} saved by ${user.username}, Version ${newVersionNumber}`);
                }
            } catch (error) {
